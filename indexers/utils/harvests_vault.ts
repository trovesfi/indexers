...
import { standariseAddress, toBigInt } from ".";
...
function processClaim(key: string, data: any[], fromAddress: string) {
  ...
  const harvestKeyUsual =
    "0x7bfb812ef65292405e9c4e05f2befe48dae3e62d7ed27bada75d2384e733d3"; // hash.getSelectorFromName("Harvest");
...
export const onEventHarvestsVault: OnEvent = async (
  event: Event,
  ...
  if (!claimInfo) {
    console.log("Skipped no valid claim info--");
  ...
  // Find contract info
  const contractInfo = (Object.keys(CONTRACTS) as ("dnmm" | "erc4626")[])
    .map((key) => {
      if (key == "erc4626") {
        return; // skip this key
      }
      if (
        CONTRACTS[key].contracts.map((c: any) => c.address).includes(contract)
      ) {
  ...
  if (
    !contractInfo &&
    key == "0x35cc0235f835cc84da50813dc84eb10a75e24a21d74d6d86278c0f037cb7429"
  ) {
  ...
  if (
    ...
    const transferEvent = allEvents.find(
      (e) =>
        ...
        e.keys[0] ===
          "0x99cd8bde557814842a3121e8ddfd4331b19826ce0696c848855790757dfa22" // Transfer event key
    );
  ...
  if (processedRecord) {
    processedRecord.amount = record.amount;
  }
