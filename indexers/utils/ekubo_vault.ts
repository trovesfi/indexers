...
export const onEventEkuboVault: OnEvent = async (
  ...
  const record: any = {
    ...
    lower_bound: (
      BigInt(positionUpdateEvent.data[7].toString()) *
      BigInt(BigInt(positionUpdateEvent.data[8]).toString() == "1" ? -1 : 1)
    ).toString(),
    upper_bound: (
      BigInt(positionUpdateEvent.data[9].toString()) *
      BigInt(BigInt(positionUpdateEvent.data[10]).toString() == "1" ? -1 : 1)
    ).toString(),
    liquidity_delta: (
      BigInt(positionUpdateEvent.data[11].toString()) *
      BigInt(BigInt(positionUpdateEvent.data[12]).toString() == "1" ? -1 : 1)
    ).toString(),
    amount0: (
      BigInt(positionUpdateEvent.data[13].toString()) *
      BigInt(BigInt(positionUpdateEvent.data[14]).toString() == "1" ? -1 : 1)
    ).toString(),
    amount1: (
      BigInt(positionUpdateEvent.data[15].toString()) *
      BigInt(BigInt(positionUpdateEvent.data[16]).toString() == "1" ? -1 : 1)
    ).toString(),
