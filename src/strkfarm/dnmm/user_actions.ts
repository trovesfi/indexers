...
export const config = {
  ...
  startingBlock: 1053807, // deployment block of first contract
  network: "starknet",
  ...
  return events
    .map(({ event, transaction }: any) => {
      ...
      const info = {
        ...
        position_acc1_supply_shares: toBigInt(
          data[4 + index_offset]
        ).toString(),
        position_acc1_borrow_shares: toBigInt(
          data[6 + index_offset]
        ).toString(),
        position_acc2_supply_shares: toBigInt(
          data[8 + index_offset]
        ).toString(),
        position_acc2_borrow_shares: toBigInt(
          data[10 + index_offset]
        ).toString(),
